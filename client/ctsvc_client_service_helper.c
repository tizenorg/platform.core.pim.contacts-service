...
int ctsvc_client_disconnect(contacts_h contact)
{
	...
	if (1 == _ctsvc_connection) {
		...
		DBG("System : connection count is %d", _ctsvc_connection);
	} else {
		DBG("System : please call contacts_connect(), connection count is (%d)", _ctsvc_connection);
		ctsvc_mutex_unlock(CTS_MUTEX_CONNECTION);
...
int ctsvc_client_disconnect_on_thread(contacts_h contact)
{
	...
	if (1 == _ctsvc_connection_on_thread) {
		...
		DBG("System : connection count is %d", _ctsvc_connection_on_thread);
	} else {
		DBG("System : please call contacts_connect_on_thread(), connection count is (%d)", _ctsvc_connection_on_thread);
		ctsvc_mutex_unlock(CTS_MUTEX_CONNECTION);
		return CONTACTS_ERROR_DB;
	}
