...
int ctsvc_db_phone_log_update_person_id(const char *number, int old_person_id, int candidate_person_id, bool person_link)
{
	...
	len = snprintf(query, sizeof(query),
			"SELECT id, number, number_type, normal_num, minmatch FROM "CTS_TABLE_PHONELOGS" "
			"WHERE log_type <= %d ",
	...
	while ((ret = ctsvc_stmt_step(get_log))) {
		...
		number_type = ctsvc_stmt_get_int(get_log, 2);
		normal_address = ctsvc_stmt_get_text(get_log, 3);
		minmatch_address = ctsvc_stmt_get_text(get_log, 4);
		...
		if (number && old_person_id <= 0 && 0 < candidate_person_id) {
			__ctsvc_db_phone_log_find_person_id(address, normal_address, minmatch_address, candidate_person_id, &new_number_type);
			new_person_id = candidate_person_id;
		} else if (number && old_person_id <= 0) {
			/* CASE : phonelog insert without person_id */
			/* address == number */
			new_person_id = __ctsvc_db_phone_log_find_person_id(address, normal_address, minmatch_address, -1, &new_number_type);
			...
			new_person_id = __ctsvc_db_phone_log_find_person_id(address, normal_address, minmatch_address, old_person_id, &new_number_type);
			...
			temp_id = __ctsvc_db_phone_log_find_person_id(address, normal_address, minmatch_address, candidate_person_id, &new_number_type);
			...
			new_person_id = __ctsvc_db_phone_log_find_person_id(address, normal_address, minmatch_address, candidate_person_id, &new_number_type);
		}
		...
		if ((new_person_id <= 0 || new_person_id == old_person_id)
			&& (new_number_type < 0 || new_number_type == number_type))
			continue;
		...
		if (0 <= new_number_type)
			ctsvc_stmt_bind_int(update_log, 2, new_number_type);
