...
#define CTSVC_AGGREGATION_SUGGESTION_MAX_CNT 3
		...
		char **sub_query, int *sub_size, int *sub_len)
{
	...
	snprintf(temp_query, CTS_SQL_MIN_LEN,
			...
			"WHERE person_id <> %d AND display_source_type=%d "
...
int ctsvc_person_get_aggregation_suggestions(int person_id, contacts_list_h *out_list)
{
	...
	while ((ret = ctsvc_stmt_step(stmt))) {
		...
		DBG("person_id : %d, score : %d", person_id, score);

		if (CTSVC_AGGREGATION_SUGGESTION_SCORE <= score
			&& cnt < CTSVC_AGGREGATION_SUGGESTION_MAX_CNT)
		{
			cnt++;
			if (sub_len)
