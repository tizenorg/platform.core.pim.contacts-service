...
static int __ctsvc_make_condition_names(contacts_record_h record, char **cond_names, int *cond_names_size, int *cond_names_len)
{
	GList *cursor = NULL;
	ctsvc_contact_s *contact = (ctsvc_contact_s*)record;
	int temp_len;
	const int comp_len = 4;
	char *name = NULL;

	name = calloc(1, comp_len + 1);
	if (NULL == name) {
		ERR("calloc() Fail");
		return CONTACTS_ERROR_OUT_OF_MEMORY;
	}

	if (contact->display_name && comp_len <= strlen(contact->display_name))
	{
		if (*cond_names_len) {
			temp_len = SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, " OR ");
			if (0 <= temp_len)
				*cond_names_len += temp_len;
		}

		temp_len = SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, "display_name");
		if (0 <= temp_len)
			*cond_names_len += temp_len;

		temp_len = SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, " LIKE ");
		if (0 <= temp_len)
			*cond_names_len += temp_len;

		temp_len = SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, "'");
		if (0 <= temp_len)
			*cond_names_len += temp_len;

		strncpy(name, contact->display_name, comp_len);
		temp_len = SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, name);
		if (0 <= temp_len)
			*cond_names_len += temp_len;
...
static int __ctsvc_make_condition_emails(contacts_record_h record, char **cond_emails, int *cond_emails_size, int *cond_emails_len)
{
	GList *cursor = NULL;
	ctsvc_contact_s *contact = (ctsvc_contact_s*)record;
	...
	for (cursor = contact->emails->records; cursor; cursor = cursor->next) {
		...
		if (email_data && email_data->email_addr && email_data->email_addr[0]) {
			if (*cond_emails_len) {
				temp_len = SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, " OR ");
				if (0 <= temp_len)
					*cond_emails_len += temp_len;
			}

			temp_len = SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, "D.data3");
			if (0 <= temp_len)
				*cond_emails_len += temp_len;

			temp_len = SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, " LIKE ");
			if (0 <= temp_len)
				*cond_emails_len += temp_len;

			temp_len = SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, "'");
			if (0 <= temp_len)
				*cond_emails_len += temp_len;

			local_part_len = strcspn(email_data->email_addr, "@");
			if (0 < local_part_len && local_part_len < strlen(email_data->email_addr)) {
				strncpy(local_part, email_data->email_addr, local_part_len+1);
				temp_len = SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, local_part);
				if (0 <= temp_len)
					*cond_emails_len += temp_len;
			} else {
				temp_len = SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, email_data->email_addr);
				if (0 <= temp_len)
					*cond_emails_len += temp_len;
			}
...
static int __ctsvc_make_condition_digits(contacts_record_h record, char **cond_digits, int *cond_digits_size, int *cond_digits_len)
{
	int ret = CONTACTS_ERROR_NONE;
	GList *cursor = NULL;
	ctsvc_contact_s *contact = (ctsvc_contact_s*)record;
		...
		if (number_data && number_data->number && number_data->number[0]) {
			...
			if (0 < ret) {
				...
				if (0 < ret)
					...
					if (CONTACTS_ERROR_NONE == ret) {
						if (*cond_digits_len) {
							temp_len = SAFE_SNPRINTF(cond_digits, cond_digits_size, *cond_digits_len, ", ");
							if (0 <= temp_len)
								*cond_digits_len += temp_len;
						}
						temp_len = SAFE_SNPRINTF(cond_digits, cond_digits_size, *cond_digits_len, "'");
						if (0 <= temp_len)
							*cond_digits_len += temp_len;

						temp_len = SAFE_SNPRINTF(cond_digits, cond_digits_size, *cond_digits_len, minmatch);
						if (0 <= temp_len)
							*cond_digits_len += temp_len;
...
static int __ctsvc_find_aggregable_person(int person_id, GSList *contact_ids, contacts_list_h *out_list)
{
	...
	int sub_query_len = 0;
	int temp_len = 0;
	int cond_digits_size = CTS_SQL_MIN_LEN;
	int cond_emails_size = CTS_SQL_MIN_LEN;
	int cond_names_size = CTS_SQL_MIN_LEN;
	int cond_digits_len = 0;
	int cond_emails_len = 0;
	int cond_names_len = 0;
	contacts_list_h list = NULL;
	cts_stmt stmt = NULL;

	RETV_IF(NULL == contact_ids || NULL == out_list, CONTACTS_ERROR_INVALID_PARAMETER);

	cond_digits = calloc(1, CTS_SQL_MIN_LEN);
	if (NULL == cond_digits) {
		ERR("calloc() Fail");
		return CONTACTS_ERROR_OUT_OF_MEMORY;
	}
	...
	for (cursor = contact_ids; cursor; cursor = cursor->next) {
		...
		int contact_id = (int)(cursor->data);
		...
		ret = __ctsvc_make_condition_digits(record, &cond_digits, &cond_digits_size, &cond_digits_len);
		ret= __ctsvc_make_condition_emails(record, &cond_emails, &cond_emails_size, &cond_emails_len);
		ret= __ctsvc_make_condition_names(record, &cond_names, &cond_names_size, &cond_names_len);

		contacts_record_destroy(record, true);
	}

	sub_query_size = CTS_SQL_MIN_LEN + cond_digits_len + cond_emails_len + cond_names_len;
	...
	if (cond_digits_len) {
		temp_len = snprintf(sub_query, sub_query_size,
					...
					"WHERE D.data4 IN (%s)",
					CTSVC_DATA_NUMBER, person_id, cond_digits);

		if (0 <= temp_len)
			sub_query_len += temp_len;
	}

	if (cond_emails_len) {
		if (sub_query_len) {
			temp_len = snprintf(sub_query + sub_query_len, sub_query_size -sub_query_len, " UNION ");
			if (0 <= temp_len)
				sub_query_len += temp_len;
		}

		temp_len = snprintf(sub_query + sub_query_len, sub_query_size -sub_query_len,
					"SELECT C.person_id FROM "CTS_TABLE_CONTACTS" C, "CTS_TABLE_DATA" D "
					"ON C.contact_id=D.contact_id AND D.datatype=%d AND C.deleted = 0 "
					"AND C.person_id <> %d AND D.is_my_profile = 0 "
					"WHERE %s",
