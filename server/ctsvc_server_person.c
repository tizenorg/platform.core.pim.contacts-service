...
static void __ctsvc_make_condition_with_names(contacts_record_h record,
		char **cond_names, int *size, int *len)
{
	ctsvc_contact_s *contact = (ctsvc_contact_s*)record;
	char comp_name[CTSVC_COMP_NAME_LEN + 1] = {0};
	char *name = NULL;
	...
	if (name && CTSVC_COMP_NAME_LEN <= strlen(name)) {
		...
		*len += SAFE_SNPRINTF(cond_names, size, *len, "reverse_sort_name");
		*len += SAFE_SNPRINTF(cond_names, size, *len, " LIKE '");
		...
		if (CTSVC_SORT_KOREAN == contact->display_name_language) { /*compare first name*/
			*len += SAFE_SNPRINTF(cond_names, size, *len, "%");
			strncpy(comp_name, name + (strlen(name) -CTSVC_COMP_NAME_LEN),
						CTSVC_COMP_NAME_LEN);
...
static void __ctsvc_make_condition_with_emails(contacts_record_h record,
		char **cond_emails, int *size, int *len)
{
	GList *cursor = NULL;
	ctsvc_contact_s *contact = (ctsvc_contact_s*)record;
...
static void __ctsvc_make_condition_with_numbers(contacts_record_h record,
		char **cond_numbers, int *size, int *len)
{
	int ret = CONTACTS_ERROR_NONE;
	GList *cursor = NULL;
	ctsvc_contact_s *contact = (ctsvc_contact_s*)record;
	...
	for (cursor = contact->numbers->records; cursor; cursor = cursor->next) {
		...
		if (number_data && number_data->number && number_data->number[0]) {
			...
			ret = ctsvc_clean_number(number_data->number, clean_num,
					sizeof(clean_num), true);
			if (ret <= 0)
				continue;
...
static int __ctsvc_make_sub_query_for_aggregation(int person_id, GSList *contact_ids,
		char **sub_query, int *sub_query_len)
{
	int ret = CONTACTS_ERROR_NONE;
	GSList *cursor = NULL;
	...
	for (cursor = contact_ids; cursor; cursor = cursor->next) {
		...
		__ctsvc_make_condition_with_numbers(record, &cond_numbers, &cond_numbers_size,
				&cond_numbers_len);
		__ctsvc_make_condition_with_emails(record, &cond_emails, &cond_emails_size,
				&cond_emails_len);
		__ctsvc_make_condition_with_names(record, &cond_names, &cond_names_size,
				&cond_names_len);

		contacts_record_destroy(record, true);
	}

	sub_query_size = CTS_SQL_MIN_LEN + cond_numbers_len + cond_emails_len
			+ cond_names_len;
	*sub_query = calloc(1, sub_query_size);
	...
	if (cond_emails_len) {
		if (*sub_query_len) {
			*sub_query_len += snprintf(*sub_query + *sub_query_len,
					sub_query_size - *sub_query_len, " UNION ");
		}

		*sub_query_len += snprintf(*sub_query + *sub_query_len,
					sub_query_size - *sub_query_len,
					"SELECT C.person_id FROM "CTS_TABLE_CONTACTS" C, "CTS_TABLE_DATA" D "
					"ON C.contact_id=D.contact_id AND D.datatype=%d AND C.deleted = 0 "
					"AND C.person_id <> %d AND D.is_my_profile = 0 "
					"WHERE %s",
...
static int __ctsvc_get_contact_ids_by_person_id(int person_id, GSList **contact_ids)
{
	int ret = CONTACTS_ERROR_NONE;
...
int ctsvc_person_get_aggregation_suggestions(int person_id, contacts_list_h *out_list)
{
	...
	ret = __ctsvc_get_contact_ids_by_person_id(person_id, &contact_ids);
	if (CONTACTS_ERROR_NONE != ret) {
		ERR("__ctsvc_get_contact_ids_by_person_id() Fail(%d)", ret);
		return ret;
	}

	ret =  __ctsvc_make_sub_query_for_aggregation(person_id, contact_ids, &sub_query,
			&sub_query_len);
	g_slist_free(contact_ids);
	if (CONTACTS_ERROR_NONE != ret) {
		free(sub_query);
		ERR("__ctsvc_make_sub_query_for_aggregation() Fail(%d)", ret);
		return ret;
	}
