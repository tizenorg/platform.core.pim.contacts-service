		...
		int *cond_names_size, int *cond_names_len)
{
	...
	if (name && CTSVC_COMP_NAME_LEN <= strlen(name)) {
		if (*cond_names_len)
			*cond_names_len += SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, " OR ");

		if (name == contact->display_name)
			*cond_names_len += SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, "display_name");
		else
			*cond_names_len += SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, "reverse_display_name");

		*cond_names_len += SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, " LIKE ");

		*cond_names_len += SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, "'");
		...
		if (CTSVC_SORT_KOREAN == contact->display_name_language) { /*compare first name*/
			...
			*cond_names_len += SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, comp_name);
		} else {
		...
		int *cond_emails_size, int *cond_emails_len)
{
	...
	for (cursor = contact->emails->records; cursor; cursor = cursor->next) {
		...
		if (email_data && email_data->email_addr && email_data->email_addr[0]) {
			...
			if (*cond_emails_len)
				*cond_emails_len += SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, " OR ");

			*cond_emails_len += SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, "D.data3");

			*cond_emails_len += SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, " LIKE ");

			*cond_emails_len += SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, "'");

			strncpy(local_part, email_data->email_addr, local_part_len + 1);
			*cond_emails_len += SAFE_SNPRINTF(cond_emails, cond_emails_size, *cond_emails_len, local_part);
		...
		int *cond_numbers_size, int *cond_numbers_len)
{
	...
	for (cursor = contact->numbers->records; cursor; cursor = cursor->next) {
		...
		if (number_data && number_data->number && number_data->number[0]) {
			...
			if (*cond_numbers_len)
				*cond_numbers_len += SAFE_SNPRINTF(cond_numbers, cond_numbers_size, *cond_numbers_len, ", ");

			*cond_numbers_len += SAFE_SNPRINTF(cond_numbers, cond_numbers_size, *cond_numbers_len, "'");

			*cond_numbers_len += SAFE_SNPRINTF(cond_numbers, cond_numbers_size, *cond_numbers_len, minmatch);
