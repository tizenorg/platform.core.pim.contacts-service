...
static int __ctsvc_make_condition_names(contacts_record_h record, char **cond_names,
		int *cond_names_size, int *cond_names_len)
{
	...
	if (name && comp_len <= strlen(name))
	{
		...
		if (name == contact->display_name)
			...
			temp_len = SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, "reverse_display_name");
		...
		if (CTSVC_SORT_KOREAN == contact->display_name_language) { /*compare first name*/
			...
			strncpy(comp_name, name + (strlen(name) -comp_len), comp_len);
			temp_len = SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, comp_name);
			if (0 <= temp_len)
				*cond_names_len += temp_len;
		} else {
			strncpy(comp_name, name, comp_len);
			temp_len = SAFE_SNPRINTF(cond_names, cond_names_size, *cond_names_len, comp_name);
			if (0 <= temp_len)
				*cond_names_len += temp_len;
...
static int __ctsvc_make_condition_emails(contacts_record_h record, char **cond_emails,
		int *cond_emails_size, int *cond_emails_len)
{
	GList *cursor = NULL;
	ctsvc_contact_s *contact = (ctsvc_contact_s*)record;
...
static int __ctsvc_make_condition_numbers(contacts_record_h record, char **cond_numbers,
		int *cond_numbers_size, int *cond_numbers_len)
{
	int ret = CONTACTS_ERROR_NONE;
	GList *cursor = NULL;
	ctsvc_contact_s *contact = (ctsvc_contact_s*)record;
	...
	for (cursor = contact->numbers->records; cursor; cursor = cursor->next) {
		...
		if (number_data && number_data->number && number_data->number[0]) {
			...
			if (*cond_numbers_len) {
				temp_len = SAFE_SNPRINTF(cond_numbers, cond_numbers_size, *cond_numbers_len, ", ");
				if (0 <= temp_len)
					*cond_numbers_len += temp_len;
			}
			temp_len = SAFE_SNPRINTF(cond_numbers, cond_numbers_size, *cond_numbers_len, "'");
			if (0 <= temp_len)
				*cond_numbers_len += temp_len;

			temp_len = SAFE_SNPRINTF(cond_numbers, cond_numbers_size, *cond_numbers_len, minmatch);
			if (0 <= temp_len)
				*cond_numbers_len += temp_len;
