...
void ctsvc_ipc_person_search_aggregable(pims_ipc_h ipc, pims_ipc_data_h indata,
		pims_ipc_data_h *outdata, void *userdata)
{
	int ret = CONTACTS_ERROR_NONE;
	int person_id = 0;
	...
	if (indata) {
		...
		ERR("ctsvc_ipc_person_search_aggregable() Fail");
		goto ERROR_RETURN;
	}
	...
	ret = ctsvc_person_search_aggregable(person_id, &list);
