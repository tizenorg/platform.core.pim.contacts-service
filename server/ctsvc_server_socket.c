...
#ifdef ENABLE_SIM_FEATURE
	have_read_permission = ctsvc_check_privilege(fd, CTSVC_PRIVILEGE_CONTACT_READ);
	if (!have_read_permission)
		INFO("fd(%d) : does not have contact read permission", fd);

	have_write_permission = ctsvc_check_privilege(fd, CTSVC_PRIVILEGE_CONTACT_WRITE);
