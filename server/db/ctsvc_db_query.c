	...
	if (0 < pro_count) {
		for (i = 0; i < pro_count; i++) {
			if (projections[i] == CTSVC_PROPERTY_PERSON_DISPLAY_NAME_INDEX) {
				...
				snprintf(temp, sizeof(temp), "temp_data._data");
				field_name = temp;
				is_data2 = true;
			} else {
		...
		contacts_list_h *out_list)
{
	...
	case CTSVC_ENUM_URI_READ_ONLY_PERSON_EMAIL:
		...
		if (range & CONTACTS_SEARCH_RANGE_NAME) {
			...
			snprintf(temp_query, sizeof(temp_query), "person_id IN (SELECT person_id "
					"FROM "CTS_TABLE_CONTACTS" WHERE deleted = 0 AND contact_id IN ");
			temp_len = SAFE_SNPRINTF(&query, &query_size, len, temp_query);
			if (0 <= temp_len)
				len += temp_len;
	...
	case CTSVC_ENUM_URI_PERSON:
		...
		if (true == is_snippet) {
			...
			char *temp_keyword = __db_get_str_with_escape((char*)keyword, strlen(keyword), true);
			...
			temp_len = SAFE_SNPRINTF(&query, &query_size, len, ")) as temp_data ON "
					"temp_data.contact_id = temp_contacts.contact_id ");
			if (0 <= temp_len)
				len += temp_len;
		}
	...
	if (STRING_EQUAL == strcmp(view_uri, CTSVC_VIEW_URI_PERSON)) {
		if (true == is_snippet) {
			temp_len = SAFE_SNPRINTF(&query, &query_size, len, " GROUP BY temp_data.contact_id");
			if (0 <= temp_len)
				len += temp_len;
		}
	}
	...
	while ((ret = ctsvc_stmt_step(stmt))) {
		...
		if (r_type == CTSVC_RECORD_PERSON) {
			...
			int ret = ctsvc_db_person_create_record_from_stmt_with_projection(stmt,
					project, ids_count, is_snippet, keyword, start_match, end_match, &record);
		...
		contacts_list_h *out_list)
{
	...
	ret = __ctsvc_db_search_records_exec(view_uri, enum_uri, p, count, projection,
			keyword, offset, limit, range, is_snippet, start_match, end_match, out_list);
	free(projection);
		...
		contacts_list_h *out_list)
{
	...
	if (STRING_EQUAL == strcmp(s_query->view_uri, CTSVC_VIEW_URI_READ_ONLY_PERSON_CONTACT)
	...
	} else if (STRING_EQUAL == strcmp(s_query->view_uri, CTSVC_VIEW_URI_PERSON)) {
		if (true == is_snippet) {
			bool need_union = false;
			char cond_data[CTS_SQL_MAX_LEN] = {0};
			char *temp_keyword = __db_get_str_with_escape((char*)keyword, strlen(keyword), true);
		...
		/*
		 * CTSVC_VIEW_URI_READ_ONLY_PERSON_NUMBER
		 * CTSVC_VIEW_URI_READ_ONLY_PERSON_EMAIL
		 */
		free(query);
		return CONTACTS_ERROR_INVALID_PARAMETER;
	}
	...
	if (STRING_EQUAL == strcmp(s_query->view_uri, CTSVC_VIEW_URI_PERSON)) {
		if (true == is_snippet) {
			temp_len = SAFE_SNPRINTF(&query, &query_size, len, " GROUP BY temp_data.contact_id");
			if (0 <= temp_len)
				len += temp_len;
	...
	while ((ret = ctsvc_stmt_step(stmt))) {
		...
		if (ctsvc_view_get_record_type(s_query->view_uri) == CTSVC_RECORD_PERSON) {
			...
			ret = ctsvc_db_person_create_record_from_stmt_with_projection(stmt, project,
					ids_count, is_snippet, keyword, start_match, end_match, &record);
			if (CONTACTS_ERROR_NONE != ret)
		...
		contacts_list_h *out_list)
{
	...
	ret = __ctsvc_db_search_records_with_query_exec(s_query, projection, condition,
			bind_text, keyword, offset, limit, is_snippet, start_match, end_match, out_list);
	if (CONTACTS_ERROR_NONE != ret) {
		...
		contacts_list_h *out_list)
{
	...
	*out_list = NULL;
	...
	return __ctsvc_db_search_records_with_range(view_uri, keyword, offset, limit, range,
			true, start_match, end_match, out_list);
}
		...
		contacts_list_h *out_list)
{
	...
	*out_list = NULL;
	...
	return __ctsvc_db_search_records_with_query(query, keyword, offset, limit, true,
			start_match, end_match, out_list);
}
