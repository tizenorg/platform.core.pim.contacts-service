...
enum {
	CTSVC_PERMISSION_NONE,
	CTSVC_PERMISSION_ENABLED,
	CTSVC_PERMISSION_DISABLED,
};
...
static void __ctsvc_set_permission_info(ctsvc_permission_info_s *info)
{
	...
	while ((ret = ctsvc_stmt_step(stmt))) {
		...
		if (!smack_enabled) { /* smack disabled */
			wi->id = id;
			wi->type = CTSVC_PERMISSION_ENABLED;
			...
			wi->id = id;
			wi->type = CTSVC_PERMISSION_DISABLED;
		} else {
			DBG("Unable to check permission");
		}

		if (wi->type == CTSVC_PERMISSION_NONE)
			free(wi);
		else
...
bool ctsvc_have_ab_write_permission(int addressbook_id, bool allow_readonly)
{
	...
	while (cursor) {
		...
		if (addressbook_id == wi->id) {
			if (CTSVC_PERMISSION_DISABLED == wi->type) {
