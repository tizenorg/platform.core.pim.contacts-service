...
int ctsvc_db_open(void)
{
	...
	if (NULL == ctsvc_db) {
		...
		if (SQLITE_OK != ret) {
			CTS_ERR("db_util_open() Fail(%d)", ret);
		...
		if (SQLITE_OK != ret) {
			CTS_ERR("sqlite3_create_function() Fail(%d)", ret);
		...
		if (SQLITE_OK != ret) {
			CTS_ERR("sqlite3_create_collation() Fail(%d)", ret);
			return CONTACTS_ERROR_DB;
		}
	}
...
int ctsvc_query_get_first_int_result(const char *query, int *result)
{
	...
	do {
		...
		if (ret != SQLITE_OK)
			CTS_ERR("sqlite3_prepare_v2() Fail(%d, %s)", ret, sqlite3_errmsg(ctsvc_db));
	...
	if (SQLITE_OK != ret) {
		CTS_ERR("sqlite3_prepare_v2() Fail(%s)", sqlite3_errmsg(ctsvc_db));
		if (ret == SQLITE_BUSY || ret == SQLITE_LOCKED)
