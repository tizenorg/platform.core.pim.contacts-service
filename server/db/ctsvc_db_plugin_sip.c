...
static int __ctsvc_db_sip_insert_record(contacts_record_h record, int *id)
{
	...
	if (CONTACTS_ERROR_NONE != ret) {
		...
		if (CONTACTS_ERROR_NO_DATA == ret) {
			CTS_ERR("No data : contact_id (%d) is not exist", sip->contact_id);
	...
	if (false == ctsvc_have_ab_write_permission(addressbook_id)) {
		CTS_ERR("Does not have permission to update this sip record : addresbook_id(%d)", addressbook_id);
		ctsvc_end_trans(false);
...
static int __ctsvc_db_sip_get_record(int id, contacts_record_h* out_record)
{
	...
	*out_record = NULL;
	...
	snprintf(query, sizeof(query),
			"SELECT id, data.contact_id, address, type, label "
...
static int __ctsvc_db_sip_delete_record(int id)
{
	...
	if (false == ctsvc_have_ab_write_permission(addressbook_id)) {
		CTS_ERR("Does not have permission to delete this sip record : addresbook_id(%d)", addressbook_id);
		ctsvc_end_trans(false);
		return CONTACTS_ERROR_PERMISSION_DENIED;
	}
