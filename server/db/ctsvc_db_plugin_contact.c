...
static inline int __ctsvc_contact_update_search_data(int contact_id, bool need_refresh_lookup_data)
{
	int ret;
	cts_stmt stmt = NULL;
	char query[CTS_SQL_MAX_LEN] = {0};
...
static int __ctsvc_db_contact_update_record(contacts_record_h record)
{
	...
	if (contact->images) {
		...
		if (count) {
			...
			default_image_path = ctsvc_utils_get_image_path(contact->image_thumbnail_path);

			if ((NULL == default_image_path && image->path) ||
					(default_image_path && NULL == image->path) ||
					(default_image_path && image->path && (STRING_EQUAL != strcmp(default_image_path, image->path)))) {
				ctsvc_record_set_property_flag((ctsvc_record_s*)contact, _contacts_contact.image_thumbnail_path, CTSVC_PROPERTY_FLAG_DIRTY);

				free(default_image_path);
				free(contact->image_thumbnail_path);
				contact->image_thumbnail_path = NULL;

				default_image_path = SAFE_STRDUP(image->path);
				if (default_image_path)
					contact->image_thumbnail_path = ctsvc_utils_make_thumbnail(default_image_path);
			}
			free(default_image_path);
...
static inline int __ctsvc_contact_insert_search_data(int contact_id, bool need_insert_lookup_data)
{
	int ret;
	cts_stmt stmt = NULL;
	char query[CTS_SQL_MAX_LEN] = {0};
...
static int __ctsvc_db_contact_replace_record(contacts_record_h record, int contact_id)
{
	...
	__ctsvc_contact_update_search_data(contact->id, true);
	ctsvc_db_update_person((contacts_record_h)contact);
