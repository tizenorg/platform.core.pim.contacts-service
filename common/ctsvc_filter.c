...
static int __ctsvc_attribute_filter_clone(ctsvc_attribute_filter_s *src, ctsvc_attribute_filter_s **dest)
{
	...
	if (src->filter_type == CTSVC_FILTER_STR)
		...
		CTS_ERR("unknown type (%d)", src->filter_type);
