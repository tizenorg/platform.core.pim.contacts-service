		...
		contacts_record_h record)
{
	...
	do {
		if (ctsvc_ipc_unmarshal_int(ipc_data, &note_p->id) != CONTACTS_ERROR_NONE)
			break;
		if (ctsvc_ipc_unmarshal_int(ipc_data, &note_p->contact_id) != CONTACTS_ERROR_NONE)
			break;
		if (ctsvc_ipc_unmarshal_string(ipc_data, &note_p->note) != CONTACTS_ERROR_NONE)
			break;
