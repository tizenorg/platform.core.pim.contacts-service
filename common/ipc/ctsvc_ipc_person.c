		...
		contacts_record_h record)
{
	...
	do {
		...
		if (ctsvc_ipc_unmarshal_string(ipc_data, &person_p->addressbook_ids) != CONTACTS_ERROR_NONE)
			break;
		if (ctsvc_ipc_unmarshal_int(ipc_data, &person_p->extra_data1) != CONTACTS_ERROR_NONE)
			break;
		if (ctsvc_ipc_unmarshal_string(ipc_data, &person_p->extra_data2) != CONTACTS_ERROR_NONE)
			break;
		...
		pims_ipc_data_h ipc_data)
{
	...
	do {
		...
		if (ctsvc_ipc_marshal_string((person_p->addressbook_ids), ipc_data) != CONTACTS_ERROR_NONE)
			break;
		if (ctsvc_ipc_marshal_int((person_p->extra_data1), ipc_data) != CONTACTS_ERROR_NONE)
			break;
		if (ctsvc_ipc_marshal_string((person_p->extra_data2), ipc_data) != CONTACTS_ERROR_NONE)
			break;
