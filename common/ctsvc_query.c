...
API int contacts_query_set_snippet(contacts_query_h query, const char *start_match, const char *end_match)
{
	ctsvc_query_s *query_s;
	RETV_IF(NULL == query, CONTACTS_ERROR_INVALID_PARAMETER);

	query_s = (ctsvc_query_s *)query;
	query_s->snippet.is_snippet = true;
	query_s->snippet.start_match = ctsvc_snippet_get_start_match(start_match);
	query_s->snippet.end_match = ctsvc_snippet_get_end_match(end_match);
