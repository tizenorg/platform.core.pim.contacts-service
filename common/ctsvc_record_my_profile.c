...
static int __ctsvc_my_profile_get_int(contacts_record_h record, unsigned int property_id, int *out)
{
	...
	case CTSVC_PROPERTY_MY_PROFILE_CHANGED_TIME:
		*out = my_profile->changed_time;
		break;
	default:
		ERR("Invalid parameter : property_id(%d) is not supported in value(my_profile)", property_id);
...
static int __ctsvc_my_profile_set_int(contacts_record_h record, unsigned int property_id, int value, bool *is_dirty)
{
	...
	case CTSVC_PROPERTY_MY_PROFILE_ADDRESSBOOK_ID:
		...
		break;
	default:
		ERR("Invalid parameter : property_id(%d) is not supported in valuecontact)", property_id);
		return CONTACTS_ERROR_INVALID_PARAMETER;
	}
	return CONTACTS_ERROR_NONE;
}
		...
		unsigned int property_id, contacts_list_h *list)
{
	...
	case CTSVC_PROPERTY_MY_PROFILE_EXTENSION:
		*list = (contacts_list_h)contact->extensions;
		break;
	default:
		ERR("Invalid parameter : property_id(%d) is not supported in value(contact)", property_id);
		return CONTACTS_ERROR_INVALID_PARAMETER;
	}
	return CONTACTS_ERROR_NONE;
}
...
static int __ctsvc_my_profile_reset_child_record_id(contacts_record_h child_record)
{
	...
	case CTSVC_RECORD_EXTENSION:
		...
		break;
	default:
		ERR("Invalid parameter : record(%d) is not child of contact", record->r_type);
		return CONTACTS_ERROR_INVALID_PARAMETER;
	}
	return CONTACTS_ERROR_NONE;
}
